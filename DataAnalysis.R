...
ggplot(data = HPIdata, aes(x = HPI, y = HPIRank)) + geom_point()  +
  aes(colour = RegionNum) + theme(legend.position = "right") + labs(title = "")
...
full_data=cbind(factor(HPIdata$Region), factor(HPIdata$Year), HPIdata$Internet, HPIdata$LifeExpect,
                HPIdata$GovHealth, HPIdata$PriHealth, HPIdata$PPP, log(HPIdata$PopDensity),HPIdata$UnEmployment,
                HPIdata$FreedomToChoice, HPIdata$Corruption)
...
# Best subset model -- Final model
model_transform <- lm (HPI ~ factor(Region) + Internet + LifeExpect + GovHealth  + log(PPP) +
                         log(PopDensity) + UnEmployment,
                       data = HPIdata.filtered)
plot(model_transform)

res <- model_transform$residuals
plot(log(HPIdata.filtered$PPP), res)
