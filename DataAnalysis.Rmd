...
#PPP
```
##Trying model with only private and gov health 
```{r}
model_gov_health <- lm (HPI ~ factor(Region) + Internet + LifeExpect + log(GovHealth) + PPP +
                         log(PopDensity),
                       data = finalData)
anova(model_gov_health)
summary(model_gov_health) #0.7815

model_pri_health <- lm (HPI ~ factor(Region) + Internet + LifeExpect + log(PriHealth) + PPP +
                         log(PopDensity),
                       data = finalData)
anova(model_pri_health)
summary(model_pri_health) #0.784 
```
...
anova(model_pri_health, model_lab_force) #p val of 0.0002039 ***
anova(model_pri_health, model_wom_parl)
```
...
anti_join(HPIdata, BaseMap, by="Country")
